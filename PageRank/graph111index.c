...
bool graph_has_edge (graph G, string vertex1, string vertex2) {
    if (G!=NULL) {
        ...
        if (N1 != NULL && N2 != NULL) {
            // check out whether arc V1->V2 existed
            // find out node of vertex2 in N1 adjlist
            ALNode targetNode;
            targetNode = N1->ALNHead;
            while (targetNode != NULL) {
...
void graph_shortest_path(graph G, string source, list ignore) {
    ...
    while(ckpt) {
        ...
        ALNode ALN1 = curNode->ALNHead;
        while (ALN1 != NULL && ALN1->nextALN != NULL) {
            ALN1 = ALN1->nextALN;
        }
        while (ALN1 != NULL) {
            if (ALN1->nodeW->exist == true) {   // not be ignored
                list_enqueue(PQueue, ALN1->nodeW->v);
                Index indexW = ALN1->nodeW->index;
                vSet[indexW] = 1;
                //针对于所有ALN1邻接的点，如果cur的dist+1 小于 or 小于等于 dist[ALN1]
                //pre[ALN1] = cur
                if (G->dist[curNode->index]+1 < G->dist[indexW]){
                    int newDist = G->dist[curNode->index]+1;
                    G->dist[indexW] = newDist;
                    G->pred[indexW] = curNode->index;
                }
